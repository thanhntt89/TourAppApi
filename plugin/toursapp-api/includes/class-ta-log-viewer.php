...
class TA_Log_Viewer {
    ...
    public static function register_menu() {
        ...
        add_action('admin_post_ta_logs_clear',      [self::class, 'handle_clear']);
        add_action('admin_post_ta_logs_delete_row', [self::class, 'handle_delete_row']);
    }
    ...
    public static function render() {
        ...
        <div class="wrap">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
                ...
                <div style="display:flex;gap:6px;align-items:center">
                    <?php
                    // Count how many logs would be deleted for default 30 days
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="ta-clear-form" style="display:flex;gap:6px;align-items:center">
                        ...
                        <button type="button" id="ta-clear-btn"
                            class="button"
                            style="<?php echo $deletable_30 === 0 ? 'opacity:0.5;cursor:not-allowed' : ''; ?>"
                            <?php echo $deletable_30 === 0 ? 'disabled' : ''; ?>
                            data-count="<?php echo $deletable_30; ?>"
                            onclick="taConfirmClear(this)">
                            🗑 Clear (<span id="ta-clear-count"><?php echo $deletable_30; ?></span>)
                        </button>
                    </form>
        ...
        var taDeleteCounts = <?php
            // Pre-compute counts for each period
            $counts = [];
        ...
        function taUpdateClearCount(days) {
            var cnt = taDeleteCounts[parseInt(days)] || 0;
        ...
        function taConfirmClear(btn) {
            ...
            if (cnt === 0) {
                btn.title = 'No logs to delete for this period.';
                return;
            }
            var box  = document.getElementById('ta-clear-confirm');
            var text = document.getElementById('ta-clear-confirm-text');
            text.textContent = '⚠️ Delete ' + cnt + ' log entries older than ' + days + ' days? This cannot be undone.';
